...
dxyf = [0 -0.5 0;-0.5 0 0.5;0 0.5 0];
weight_xy = ones(n-2,m-2);
% dxf=[1 -1];
% dyf=[1;-1];
% dyyf=[-1; 2; -1];
% dxxf=[-1, 2, -1];
...
[D]=deconvL2_w(D(hfs_y1+1:n-hfs_y2,hfs_x1+1:m-hfs_x2),filt1,we,max_it,weight_xy);
...
for t=1:2
  ...
  dxyD=conv2(D,fliplr(flipud(dxyf)),'valid');

  weight_xy=lambda_f*2/(2*thr_e^2+(dxyD).^2); 
...
D=D(hfs_y1+1:n-hfs_y2,hfs_x1+1:m-hfs_x2);
